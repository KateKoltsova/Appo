...
watch(selectedCategories, (newValue, oldValue) => {
  if (newValue.length > 0 && newValue != oldValue && selectedDate.value) {
    getServices();
...
const addToCart = (schedule) => {
  ...
  if (userId != null) {
    add(userId, JSON.stringify(item));
  } else {
  ...
  console.log(
    `Добавлено в корзину: Расписание ID ${item.schedule_id}, Услуга ID ${item.service_id}, Цена ID ${item.price_id} ${item}`
  );
};
</script>
